...
class Sma
{
    ...
    public function save_barcode($text = null, $bcs = 'code128', $height = 30, $stext = 1, $get_be = false)
    {
    ...
    {
        $file_name = 'assets/upload/qrcode' . $this->session->userdata('user_id') . ($sq ? $sq : '') . ($this->Settings->barcode_img ? '.png' : '.svg');
        if ($type == 'link') {
            $text = urldecode($text);
        }
    ...
    public function generate_pdf($orientation = 'P',$content, $name = 'download.pdf', $output_type = null, $footer = null, $margin_bottom = null, $header = null, $margin_top = null)
    {
        if (!$output_type) {
            $output_type = 'D';
        }
        ...
        $stylesheet = file_get_contents('assets/css/bootstrap3.min.css');
        ...
        if ($output_type == 'S') {
            ...
            write_file('assets/upload/' . $name, $file_content);
