...
<div class="container-fluid">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"> <?= display('add_services') ?> </h5>
                ...
                <form class="needs-validation">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label> <?=display('service_name')?> <span class="text-danger error_lastname">*</span></label>
                            <input type="text" class="form-control" placeholder="" name="nameservice" id="nameservice" autocomplete="off">
                        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form class="needs-validation">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-primary" data-dismiss="modal"><?=display('close')?></button>
                        <button type="button" id="savebtn" onclick="updatebtm()" class="btn btn-primary"><?=display('update')?></button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-12">
            <h1> <?=display('additional_services')?></h1>
            <nav class="breadcrumb-container d-lg-inline-block mb-sm-0 mb-2" aria-label="breadcrumb">
                <ol class="breadcrumb pt-0">
                    <li class="breadcrumb-item">
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-backdrop="static" data-target="#exampleModalRight"><i class="fa fa-plus"></i> Services</button>
                    </li>
                    <li class="breadcrumb-item">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('#table').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('services/ajax_service/') ?>",
                "type": "POST",
                "data": function(data) {},
            },
    ...
    function deletebtm(idClt) {
        $.ajax({
            url: "<?php echo base_url('services/delete/') ?>" + idClt,
            type: "POST",
            dataType: "JSON",
            data: {},
            success: function(data) {
    ...
    // update
    function updatebtm() {
        var id = $('#idserveup').val();
        var num = $('#numup').val();
        var nameservice = $('#nameserviceup').val();
        if (nameservice != '') {
            $.ajax({
                url: "<?php echo base_url('services/update_ajax') ?>",
                type: "POST",
                dataType: "JSON",
                data: {
                    id: id,
