			...
			<div class="row justify-content-center">
				<div class="col-md-4 mx-auto" style="margin: 0 auto;">
				    <?php foreach ($sale_items as $item): 
                        ...
                        for($i=1;$i<=$items;$i++):
                    ?>
					<div class="content_area ticket" style="border: 1px solid;padding:5px; width:250px;  text-align:center; margin:0 auto;">
						<div class="logo">
						...
						<div class="text_area">
							<div class="row">
								<div class="col-sm-8">
									<div class="txt">
									    	<p style="margin-bottom:4px; font-size:16px; font-weight:600;">Order ID.<?=$item->sale_id?></p>
									    	
										    <p style="margin-bottom:4px; font-size:16px; font-weight:600;">Customer ID. ( CL<?=$customer->id?> )</p>
									    	<div style="padding-bottom:5px; font-weight:600;" class="date_time"><?=date('d-M-Y')?></div>
									</div>
								</div>
								<!--<div class="col-sm-4">-->
								<!--	<div class="qr_code">-->
							...
							<div class="description" style="padding-top:5px;">
								...
								<!--<p style="margin:0; padding:0;"><b><?=date('d.M.Y',strtotime($sale->delivery_date)).' '. date('M',strtotime($item->delivery_date))?></b></p>-->
								<p style="margin:0; padding:0;"><b><?=date('d.M.Y',strtotime($sale->delivery_date))?></b></p>
								<p><b><?=$customer->firstname.' '.$customer->lastname?></b></p>
							</div>

							<div class="img_area">
								<div class="img_inner" style="margin-top:4px; border: 1px solid #727272; padding:5px;  width:96%; margin: 0 auto;"> 
								 
								   
									<?php
                                    if ($item->secondary_icons) {
                                        ?>
                                        <div style="width: 100%; display:flex; flex-wrap: wrap; gap:2px;">
                                            <img style="width:24.2%; object-fit:cover;" src="https://laundroklean.meshink.xyz/super-admin/<?=$item->service_icon?>" alt="Primary Service Icon" />
                                            <?php
                                            if (!empty($item->secondary_icons)) {
                                                $decoded_icons = json_decode($item->secondary_icons, true);
                                                if (is_array($decoded_icons)) {
                                                    foreach ($decoded_icons as $icon_url) {
                                                        $icon_path = "https://laundroklean.meshink.xyz/super-admin/" . $icon_url;
                                                        echo "<img style='width: 24.2%; object-fit: cover;' src='$icon_path' alt='Secondary Service Icon' />";
                                                    }
                                                }
                                            }
                                            ?>
                                        ...
                                        ?>
                                        <img style="width:24.2%; object-fit:cover;" src="https://laundroklean.meshink.xyz/super-admin/<?=$item->service_icon?>" alt="Service Icon" />
                                        <?php
                                    }
                                    ?> 
								...
								<div class="img_text">
									<p style=" font-size:18px;font-weight:500;  padding:0; margin:0;"><?=$item->name?></p>
								</div>
							</div>
							
							<div class="barcode_img">
								<img style="height:80px;" src="<?=base_url() . 'assets/images/barcode/' . $item->barcode.'.png'?>" alt="..." />
							</div>

							<div class="footer">
								<div class="footer_title">
										<p style="margin-bottom:5px; font-size:24px; font-weight:600;"><?=$this->session->userdata('store_code')?></p>
								</div>
								<div class="footer_text">
									<img class="phone" src="<?=base_url()?>assets/images/phone.png" alt="..." />
									<p><?=$store_info->store_phone?></p>
								</div>
							</div>
						</div>
					</div>
                        ...
                        printWindow.document.write(`
                            ...
                            <head>
                                ...
                                <style>
                                    ...
                                    .content_area { width: 250px; margin: auto; border: 1px solid black; padding: 10px; }
                                    .barcode_img img { width: 100%; height: auto; } /* Ensure barcode images load correctly */
                                </style>
