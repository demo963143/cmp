...
class Backup extends CI_Controller {
	...
	*/
	function index() {
		...
		$data['backups'] = $this->backup->get_all();
		$data['content_page']=$this->load->view('backup/backup' ,$data,true);
		...
		if ($file_format == 1) {
			...
			if (strlen($backup)) {
				if (!write_file($file, $backup)) {
					$this->handle_error(display('error_writing_a_backup_db_copy_to_disk') . $file_name);
				} else {
					...
					if ($saved_data !== NULL) {
						...
						$this->handle_success('You can download db backup here' . anchor($this->db_download_url . $saved_data, 'تحميل', array('class' => 'btn btn-primary d-block mt-2 w-25')));
						$this->handle_success('You can delete db backup here ' . anchor($this->db_delete_url . $saved_data, 'حدف', array('class' => 'btn btn-danger d-block mt-2 w-25', 'onclick' => "return confirm('Are you sure want to delete this file ?')")));
					} else {
						if (file_exists($file)) {
							unlink($file);
						}
