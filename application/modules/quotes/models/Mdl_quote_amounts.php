...
class Mdl_Quote_Amounts extends CI_Model
{
    ...
    public function get_status_totals($period = '')
    {
     ...
     public function quotes_count()
    {
           return $this->db->select('count(*)as quotes')->from('ip_quotes')->where('DATE(quote_date_created)',date("Y-m-d"))->get()->row();
    }
  /*   public function quotes_count_by_date($start_date,$end_date)
    {
           return $this->db->select('count(*)as quotes')->from('ip_quotes')
                ->where('DATE(quote_date_created) BETWEEN "'. date('Y-m-d', strtotime($start_date)). '" and "'. date('Y-m-d', strtotime($end_date)).'"')->get()->row();
    }*/   
    
    public function quotes_count_by_date($start_date=null,$end_date=null,$search=null)
    {
      if($search)
        {
            $start_date="";
