...
class Ajax extends Admin_Controller
{
    ...
    {
		$date = $_REQUEST['invoice_date_created'];
		$year = date('Y', strtotime($date));
		$month = date('m', strtotime($date));
		
				print_r($condition);
		$this->db->where('YEAR(invoice_date_created)',$year);
		$this->db->where('MONTH(invoice_date_created)',$month);
		$query=$this->db->get('ip_invoices')->result_array();
		$count=count($query);
		if($count<=2020)
		{
			$this->load->model('invoices/mdl_invoices');
			...
			$response="Limit of Invoice is exceeded";
		}
			echo json_encode($response);
    }
