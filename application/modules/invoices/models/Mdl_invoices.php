...
class Mdl_Invoices extends Response_Model
{
    ...
    public function statuses()
    {
        return array(
            ...
            '7' => array(
                ...
                'class' => 'inprogress',
                'href' => 'invoices/status/inprogress'
            ),
            '8' => array(
                'label' => trans('Pending'),
                'class' => 'pending',
    ...
    {
           return $this->db->select('count(*)as inv_count')->from('ip_invoices')->where('DATE(invoice_date_created)',date("Y-m-d"))->get()->row();
    }
  /*  public function invoice_count_by_date($start_date,$end_date)
    {
           return $this->db->select('count(*)as inv_count')->from('ip_invoices')
                ->where('DATE(invoice_date_created) BETWEEN "'. date('Y-m-d', strtotime($start_date)). '" and "'. date('Y-m-d', strtotime($end_date)).'"')->get()->row();
    }*/

    public function invoice_count_by_date($start_date=null,$end_date=null,$search=null)
    {
      if($search)
        {
            $start_date="";
