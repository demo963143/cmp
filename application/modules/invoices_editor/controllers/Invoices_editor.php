 ...
 * Class Invoices
 */
class Invoices_editor extends Editor_Controller
{
    ...
    public function index()
    {
        ...
        redirect('invoices_editor/status/all');
    }
    ...
    public function status($status = 'all', $page = 0)
    {
        ...
        $this->mdl_invoices->paginate(site_url('invoices_editor/status/' . $status), $page);
        ...
        $this->layout->buffer('content', 'invoices_editor/index');
        $this->layout->render('layout_editor');
    }
    ...
    public function archive()
    {
        ...
        if (isset($_POST['invoice_number'])) {
            ...
            $this->layout->buffer('content', 'invoices_editor/archive');
           $this->layout->render('layout_editor');
    ...
    public function view($invoice_id)
    {
        $this->load->model(
            array(
                'clienteditor/mdl_items',
                'clienteditor/mdl_tax_rates',
                'clienteditor/mdl_payment_methods',
                'clienteditor/mdl_invoice_tax_rates',
                'clienteditor/mdl_custom_fields',
            )
        );

        $this->load->helper("custom_values");
        $this->load->helper("client");
        $this->load->model('clienteditor/mdl_units');
        $this->load->module('payments_editor');

        $this->load->model('clienteditor/mdl_custom_values');
        $this->load->model('clienteditor/mdl_invoice_custom');
        ...
        if ($invoice->sumex_id != null) {
            $this->layout->buffer(
                array(
                    array('modal_delete_invoice', 'invoices_editor/modal_delete_invoice'),
                    array('modal_add_invoice_tax', 'invoices_editor/modal_add_invoice_tax'),
                    array('modal_add_payment', 'payments_editor/modal_add_payment'),
                    array('content', 'invoices_editor/view_sumex')
                )
            );
        } else {
            $this->layout->buffer(
                array(
                    ...
                    array('modal_add_invoice_tax', 'invoices_editor/modal_add_invoice_tax'),
                    array('modal_add_payment', 'payments_editor/modal_add_payment'),
                    array('content', 'invoices_editor/view')
                )
            );
        }
   ...
   $this->layout->render('layout_editor');
    ...
    public function delete($invoice_id)
    {
        ...
        if ($invoice_status == 1 || $this->config->item('enable_invoice_deletion') === true) {
            ...
            $this->load->model('clienteditor/mdl_tasks');
        ...
        redirect('invoices_editor/index');
    }
