...
class Mdl_Invoices extends Response_Model
{
	...
	public function defaultresult(){
		$result = $this->db->query("SELECT Sum(i_i_a.`invoice_total`),i_c.`client_name` FROM `ip_invoices` inum LEFT JOIN `ip_invoice_amounts` i_i_a ON i_i_a.`invoice_id`=inum.`invoice_id` LEFT JOIN `ip_clients` i_c ON i_c.`client_id` = inum.`client_id`  GROUP BY inum.`user_id`");
	...
	public function gettotalinvoice($invoice_id){
