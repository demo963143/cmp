...
class Mdl_Tasks extends Response_Model
{
    ...
    {
           return $this->db->select('count(*)as tasks')->from('ip_tasks')->where('DATE(task_creation_date)',date("Y-m-d"))->get()->row();
    }
 /*    public function task_count_by_date($start_date,$end_date)
    {
           return $this->db->select('count(*)as tasks')->from('ip_tasks')
                ->where('DATE(task_creation_date) BETWEEN "'. date('Y-m-d', strtotime($start_date)). '" and "'. date('Y-m-d', strtotime($end_date)).'"')->get()->row();
    }*/
     public function task_count_by_date($start_date=null,$end_date=null,$search=null)
    {
      if($search)
        {
            $start_date="";
