...
class Reports extends Admin_Controller
{
    ...
    public function activities()
    {
        if ($this->input->post('btn_submit')) {
            $data = array(
                'results' => $this->mdl_reports->activities($this->input->post('from_date'), $this->input->post('to_date')),
                'from_date' => $this->input->post('from_date'),
                'to_date' => $this->input->post('to_date'),
            );
               // print_r($data);
              //  die;
            $html = $this->load->view('reports/activities', $data, true);

            $this->load->helper('mpdf');

            pdf_create($html, trans('activities'), true);
        }

        $this->layout->buffer('content', 'reports/activities_index')->render();
    }
    public function login_logs()
    {
        if ($this->input->post('btn_submit')) {
            $data = array(
                'results' => $this->mdl_reports->login_logs($this->input->post('from_date'), $this->input->post('to_date')),
                'from_date' => $this->input->post('from_date'),
                'to_date' => $this->input->post('to_date'),
            );
               // print_r($data);
              //  die;
            $html = $this->load->view('reports/login_logs', $data, true);

            $this->load->helper('mpdf');

            pdf_create($html, trans('login_logs'), true);
        }

        $this->layout->buffer('content', 'reports/login_logs_index')->render();
    }
