...
class Mdl_Reports extends CI_Model
{
    ...
    public function activities($from_date = null, $to_date = null)
    {
        if ($from_date and $to_date) {
          //  die($from_date);
            $from_date = date_to_mysql($from_date);
           ...
           $query= $this->db->select('*')->from('ip_client_activities')
                     ->join('ip_clients', 'ip_client_activities.client_id=ip_clients.client_id','LEFT')
             ->where('DATE(act_created_at) BETWEEN "'. $from_date.'" and "'.$to_date.'"');
        }
        $data= $query->get()->result();
        return $data;
    }
    public function login_logs($from_date = null, $to_date = null)
    {
        if ($from_date and $to_date) {
          //  die($from_date);
            $from_date = date_to_mysql($from_date);
           ...
           $query= $this->db->select('*')->from('ip_login_logs')
             ->where('DATE(log_out_time) BETWEEN "'. $from_date.'" and "'.$to_date.'"');
        }
        $data= $query->get()->result();
        return $data;
    }
